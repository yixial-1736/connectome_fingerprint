...
prediction = os.path.join(SaveDir, 'rh_' + Contrast2 + 'pred_' + subname + '.csv')
np.savetxt(prediction, y_pred, delimiter= ',')

data = zip([a], [subname], [r], [p])
filename = os.path.join(SaveDir, 'rh_' + Contrast2 + '_' + subname + '.csv')
filename2 = os.path.join(SaveDir, 'rh_' + Contrast2 + '_Coef' + subname + '.csv')
